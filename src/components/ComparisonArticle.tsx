...
export function ComparisonArticle({ comp, path }: ComparisonArticleProps) {
  const image = getComparisonImage(comp.slug);
        ...
        <div className="container-wide max-w-4xl">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {isGuide
              ...
              : comp.relatedSlugs.map((s) => (
                  <Link key={s} href={comparisonPath(s)} className="btn-secondary text-sm">
                    Related comparison
                  </Link>
                ))}
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {comparisons
              ...
              .slice(0, 4)
              .map((c) => (
                <Link key={c.slug} href={comparisonPath(c.slug)} className="nav-card">
