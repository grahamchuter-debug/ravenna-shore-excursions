...
interface GuideArticleProps {
  page: GuidePage;
  hubLabel: string;
  getRelatedPage: (slug: string) => RelatedGuideLink | undefined;
}

export function GuideArticle({ page, hubLabel, getRelatedPage }: GuideArticleProps) {
  const path = guidePath(page.slug);
  const image = subjectImages[page.imageKey] ?? subjectImages["historic-centre"];
  const related = page.relatedSlugs.map(getRelatedPage).filter(Boolean) as RelatedGuideLink[];
