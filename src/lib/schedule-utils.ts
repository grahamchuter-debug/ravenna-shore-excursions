...
/** Single-port site — every schedule path resolves under /ravenna-cruise-schedule. */
const SCHEDULE_HUB = "/ravenna-cruise-schedule";

export function yearHubPath(year: ScheduleYear): string {
  return `${SCHEDULE_HUB}/${year}`;
}
...
export function portMonthPath(_slug: string, monthKey: string): string {
  return `${SCHEDULE_HUB}/${monthKeyToSlug(monthKey)}`;
}
